module PFM
  class Pokemon
        # Return the normalized text trainer id of the Pokemon when not shadow
    # @return [String]
    def trainer_id_text
      return self.shadow? ? "??????" : sprintf("%05d", self.trainer_id)
    end

    # Return the trainer name of the Pokemon when not shadow
    # @return [String]
    def trainer_name_text
      return self.shadow? ? "??????" : "#{self.trainer_name}"
    end
  end
end

# Move the text displaying the "EXP" and stuff to load_text_info instead of init_sprite
module UI
  ...
  class Summary_Memo < SpriteStack
    ...
    def init_memo
      ...
      no_egg add_text(114, 19 + 16 * 3, 92, 16, :trainer_name_text, 2, type: SymText, color: 1)
