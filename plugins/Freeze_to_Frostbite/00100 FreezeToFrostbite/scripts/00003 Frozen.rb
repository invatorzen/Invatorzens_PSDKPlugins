      ...
      class Frozen < Status
        ...
        def on_status_prevention(handler, status, target, launcher, skill)
          ...
          return handler.prevent_change do
            handler.scene.display_message_and_wait(parse_text_with_pokemon(400000, PREVENT_MESSAGE_LINE - 2, target)) # Message shown if the target already is frostbitten
          end
        end
        ...
        def on_end_turn_event(logic, scene, battlers)
          ...
          scene.display_message_and_wait(parse_text_with_pokemon(400000, PREVENT_MESSAGE_LINE - 8, target)) # Message shown after you take damage from frostbite
